...
Relational Databases store related data in separated tables, linking them through primary and foreign keys allowing the data to become less redundant.
SQL is a language used to perform queries on relational databases. It is not a database management system, but it is closely linked to relational database management systems because it can manage data in RDBMS.

- [X] Why do tables need a `primary key`?

Tables need a primary key so that they can be distinguished form each other. A primary key must be unique so that data can be referenced individually. Without a primary key, foreign keys can't be created, and relational databases cannot exist without relationships between tables.
...
Many to many relationships cannot exist as it would require a varying amount of foreign keys and a varying amount of columns. To create one, a table that stores the relationships between these tables must be created having a column for each foreign key to the tables. This allows us to store rows of relationships.
